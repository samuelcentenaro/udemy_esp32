...
// Queue handle used to manipulated the main queue events
...
/**
 * @brief Initialize the WiFi application event handler for WiFi and IP events.
 */
static void
wifi_app_event_handler_init(void)
{
  // Evento loop for the WiFi driver
  ESP_ERROR_CHECK(esp_event_loop_create_default());

  // Evet handler for the connection
...
/**
 * @brief Initializer the TCP stack and default WiFi configutation
 */
...
/**
 * @brief Connect the ESp32 to an external AP using the update station configuration
 */
 ...
 * @param pvParameters parameer which can be passed to the task
 * @return BaseType_t
 */
  ...
  for (;;)
  {
    ...
    if (xQueueReceive(wifi_app_queue_handle, &msg, portMAX_DELAY))
    {
      ...
      case WIFI_APP_MSG_LOAD_SAVED_CREDENTIALS:
        ...
        {
          ESP_LOGI(TAG, "Loaded saved credentials");
      ...
      case WIFI_APP_MSG_START_HTTP_SERVER:
        ESP_LOGI(TAG, "Starting WIFI_APP_MSG_START_HTTP_SERVER");
        http_server_start();
      ...
      case WIFI_APP_MSG_CONNECTING_FROM_HTTP_SERVER:
        ...
        wifi_app_connect_sta(); // Attempt a connection
        g_retry_number = 0;     // Set current number of retries to zero
      ...
      case WIFI_APP_MSG_STA_CONNECTED_GOT_IP:
        ...
        if (eventBits & WIFI_APP_CONNECTING_FROM_HTTP_SERVER_BIT) // If connecting from the http server, then clear the bit
        {
      ...
      case WIFI_APP_MSG_USER_REQUESTED_STA_DISCONNECT:
        ...
        rgb_led_http_server_started(); ///> todo: rename this status LED to a name mode meaninful ...
      ...
      case WIFI_APP_MSG_STA_DISCONNECTED:
        ...
        {
          ESP_LOGI(TAG, "WIFI_APP_MSG_STA_DISCONNECTED: attempt connection from http server");
          xEventGroupClearBits(wifi_app_event_group, WIFI_APP_CONNECTING_FROM_HTTP_SERVER_BIT);
        ...
        {
          ESP_LOGI(TAG, "WIFI_APP_MSG_STA_DISCONNECTED: attempt FAILED, check WiFi acess point availability");
          // Adust this case to your nedds - maybe you want to keep trying to connect...
        }
        break;

      default:
        ESP_LOGE(TAG, "Unknown event %d", msg.msgID);
        break;
      }
    }
  }
...
{
  ESP_LOGI(TAG, "Starting WIFI_APP");

  // Start WIFI started LED RGB
