...
// Tag used for ESP serial output
...
// Queue handle user to manipulate the main queue events
static QueueHandle_t http_server_monitor_queue_handle;

/**
 * @brief ESP32 timer configutarion passed to esp_timer_create.
 */
const esp_timer_create_args_t fw_update_reset_arg = {
...
// Embedded files: Jquery, app.css, app.js, index.html, favicon.ico files
...
static void http_server_fw_update_reset_timer(void)
{
  if (g_fw_update_status == OTA_UPDATE_SUCCESSFUL)
  {
    ...
    // Give the web page a chance to receive and ackowledge back and initialize the timer
    ESP_ERROR_CHECK(esp_timer_create(&fw_update_reset_arg, &fw_update_reset));
    ESP_ERROR_CHECK(esp_timer_start_once(fw_update_reset, 800000));
  }
  else
  {
    ESP_LOGI(TAG, "http_server_fw_update_reset_timer: FW update not successful");
  }
}

/**
 * @brief HTTP server monitor task used to track events of the HTTP server
 * @param pvParameters parameter wich can be passed to the task
 */
 ...
 * @param req HTPP request for wich the uri needs to be handled.
 * @return ESP_OK
 */
...
/**
 * @brief Index html get handler requested when accessing the web page
 * @param req HTPP request for wich the uri needs to be handled.
 * @return ESP_OK
 */
...
/**
 * @brief app.css get handler is requested when accessing the web page
 * @param req HTPP request for wich the uri needs to be handled.
 * @return ESP_OK
 */
...
/**
 * @brief app.js get handler is requested when accessing the web page
 * @param req HTPP request for wich the uri needs to be handled.
 * @return ESP_OK
 */
...
/**
 * @brief Receives .bin file via the web page and handles the firmware update
...
/**
 * @brief DHT Sensor readings JSON handler responds with DHT22
 * @param req HTTP request for which the uri needs to be handled
 * @return ESP_OK
 */
...
/**
 * @brief wifiConnect.json handler is invoked after the connect button is pressed
 * and handles receiving the SSID and password entered by user
 * @param req HTTP request for which the uri needs to be handled
 * @return ESP_OK
 */
static esp_err_t http_server__wifi_connect_json_handler(httpd_req_t *req)
{
  ...
  if (len_ssid > 1)
  {
    ...
    {
      ESP_LOGI(TAG, "http_server__wifi_connect_json_handler: Found header => My-connect-SSID: %s", ssid_str);
    }
  }
...
/**
 * @brief  wifiConnectInfo.json handler updates the web page with connection information
 * @param req HTTP request for which the uri needs to be handled
 * @return ESP_OK
 */
static esp_err_t http_server_wifi_get_connect_info_handler(httpd_req_t *req)
{
...
{
  // Genarate default configuration
  ...
  // Core that will be used by the server
  config.core_id = HTTP_SERVER_TASK_CORE_ID;

  // Adjust the defalut priority to 1 less than the WiFi application
  ...
  ESP_LOGI(TAG, "http_server_configure: Starting HTTP server on port: %d whit task priority: %d ", config.server_port, config.task_priority);
  ...
  if (httpd_start(&http_server_handle, &config) == ESP_OK)
  {
    ...
    httpd_uri_t wifi_connect_json = {
        ...
        .method = HTTP_POST,
        .handler = http_server__wifi_connect_json_handler,
        .user_ctx = NULL};
...
{
  ESP_LOGI(TAG, "http_server_fw_update_reset_callback: Timer time-out, restarting the device");
  esp_restart();
}